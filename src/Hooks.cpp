	...
	while (true)
	{
		...
		for (decltype(auto) it : AntiFlood)
		{
			if (!std::get<0>(it.second))
			{
				...
				{
					if (rnd == 1) sendMTAChat(xorstr_("b"), cp1251_to_utf8(xorstr_("“ут €")).c_str(), true, false, false);
					else if (rnd == 2) sendMTAChat(xorstr_("b"), cp1251_to_utf8(xorstr_("я тут")).c_str(), true, false, false);
					else if (rnd == 3) sendMTAChat(xorstr_("b"), cp1251_to_utf8(xorstr_("«десь а что?")).c_str(), true, false, false);
					else if (rnd == 4) sendMTAChat(xorstr_("b"), cp1251_to_utf8(xorstr_("здесь, здесь")).c_str(), true, false, false);
					else if (rnd == 5) sendMTAChat(xorstr_("b"), cp1251_to_utf8(xorstr_("“ут, тут...")).c_str(), true, false, false);
				}
				else
				{
					if (rnd == 1) sendMTAChat(xorstr_("say"), cp1251_to_utf8(xorstr_("“ут €")).c_str(), true, false, false);
					else if (rnd == 2) sendMTAChat(xorstr_("say"), cp1251_to_utf8(xorstr_("я тут")).c_str(), true, false, false);
					else if (rnd == 3) sendMTAChat(xorstr_("say"), cp1251_to_utf8(xorstr_("«десь а что?")).c_str(), true, false, false);
					else if (rnd == 4) sendMTAChat(xorstr_("say"), cp1251_to_utf8(xorstr_("здесь, здесь")).c_str(), true, false, false);
					else if (rnd == 5) sendMTAChat(xorstr_("say"), cp1251_to_utf8(xorstr_("“ут, тут...")).c_str(), true, false, false);
				}
...
unsigned long ulFormat, void* pDXFont, bool bOutline)
{
	if (szText != nullptr)
	{
		...
		if (findStringIC(txt.c_str(), xorstr_("Admin")) || findStringIC(txt.c_str(), xorstr_("јдмин")))
		{
			if (sendMTAChat)
			{
